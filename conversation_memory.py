"""
==============================================================================
CONVERSATION_MEMORY.PY - ПАМЯТЬ ДИАЛОГА И СОСТОЯНИЕ РАЗГОВОРА
==============================================================================

ОПИСАНИЕ:
    Хранит историю диалога и состояние multi-turn разговоров.
    Использует скользящее окно для контекста (последние N сообщений).

ЗАВИСИМОСТИ (импортирует):
    - database.py (передаётся в конструктор) → для работы с БД

ИСПОЛЬЗУЕТСЯ В (импортируется в):
    - bot.py             → conversation_memory = ConversationMemory(db)
    - conversation_handler.py → для get_state(), set_state(), clear_state()

КЛЮЧЕВОЙ КЛАСС: ConversationMemory

ТАБЛИЦЫ БД (используемые):
    - conversation_sessions → История сообщений (role, content, intent)
    - conversation_state    → Состояние multi-turn диалога (pending_action, partial_data)

КЛЮЧЕВЫЕ МЕТОДЫ:
    | Метод                | Описание                              |
    |----------------------|---------------------------------------|
    | add_message()        | Добавить сообщение в историю          |
    | get_history()        | Получить историю (скользящее окно)    |
    | get_state()          | Получить состояние диалога            |
    | set_state()          | Установить состояние (ожидание ответа)|
    | clear_state()        | Очистить состояние после выполнения   |
    | update_partial_data()| Обновить одно поле в partial_data     |
    | cleanup_old_messages()| Удалить старые сообщения (>30 дней)  |

СТРУКТУРА STATE:
    {
        'pending_action': 'event',      # Ожидаемое действие
        'partial_data': {...},          # Собранные данные
        'awaiting_field': 'start_time', # Какое поле ждём
        'context_event_id': 123,        # ID связанного события
        'updated_at': datetime          # Время последнего обновления
    }

ВАЖНЫЕ ОСОБЕННОСТИ:
    1. Таймаут состояния: 10 минут (600 секунд). После - автоочистка.
    2. История диалога: по умолчанию 20 последних сообщений.
    3. partial_data сериализуется в JSON для хранения в БД.
...
class ConversationMemory:
    ...
    Хранит историю сообщений для контекста NLP и состояние multi-turn
    диалогов (когда бот задаёт уточняющие вопросы).
    
    Attributes:
        db: Database - инстанс базы данных для хранения данных
    """

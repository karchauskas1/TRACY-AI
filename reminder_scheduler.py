...
ОПИСАНИЕ:
    Фоновый планировщик для отправки напоминаний о событиях и утреннего дайджеста.
    Работает в бесконечном цикле, проверяя напоминания каждую минуту.

ЗАВИСИМОСТИ (импортирует):
    - database.py   → get_pending_reminders(), mark_reminder_sent()
    - config.py     → OPENROUTER_API_KEY для генерации мотивационных цитат

ИСПОЛЬЗУЕТСЯ В (импортируется в):
    - bot.py        → reminder_scheduler = ReminderScheduler(bot, db, ai_client)
    - decision_engine.py → reminder_scheduler.create_reminders_for_event()

КЛЮЧЕВОЙ КЛАСС: ReminderScheduler

MAIN LOOP:
    _check_reminders_loop() → Бесконечный цикл (интервал: 60 секунд)
        ├── _check_and_send_reminders() → Проверка и отправка напоминаний
        └── _check_and_send_morning_digests() → Утренний дайджест

ТИПЫ НАПОМИНАНИЙ (reminder_type):
    - 'before'   → За N минут до события (стандартное)
    - 'start'    → В момент начала события
    - 'interval' → Интервальные напоминания (каждые N минут)

УТРЕННИЙ ДАЙДЖЕСТ:
    - Отправляется в настроенное время (morning_digest_time из user_settings)
    - Содержит список событий на день
    - Включает AI-сгенерированную мотивационную цитату
...
КЛЮЧЕВЫЕ МЕТОДЫ:
    ...
    | create_reminders_for_event() | Создание напоминаний для события      |
    | _send_reminder()             | Отправка одного напоминания           |
    | _generate_motivational_quote()| Генерация цитаты через AI            |

ВАЖНЫЕ ОСОБЕННОСТИ:
    1. Проверяет напоминания в UTC, конвертирует в часовой пояс пользователя.
    2. Просроченные напоминания (до 30 минут) всё равно отправляются.
    3. Напоминание помечается sent=True только после успешной отправки.
    4. morning_digest_time = NULL означает дайджест выключен.
...
class ReminderScheduler:
    """
    Фоновый планировщик для отправки напоминаний и утреннего дайджеста.
    
    Работает в отдельной asyncio задаче, проверяя БД каждую минуту.
    
    Attributes:
        bot: ExtBot - инстанс бота для отправки сообщений
        db: Database - инстанс БД
        running: bool - флаг работы планировщика
        check_interval: int - интервал проверки (60 сек по умолчанию)
        ai_client: OpenAI - для генерации мотивационных цитат
    """

"""
==============================================================================
RECURRING_EVENTS.PY - ДВИЖОК ПОВТОРЯЮЩИХСЯ СОБЫТИЙ (WIP)
==============================================================================

ОПИСАНИЕ:
    Модуль для управления повторяющимися событиями (ежедневные, еженедельные, ежемесячные).
    ⚠️ СТАТУС: В РАЗРАБОТКЕ (WIP) - не все методы реализованы.

ЗАВИСИМОСТИ (импортирует):
    - database.py       → Database для сохранения событий

ИСПОЛЬЗУЕТСЯ В (импортируется в):
    - (планируется) decision_engine.py → для создания recurring events

КЛЮЧЕВОЙ КЛАСС: RecurringEventsEngine

ТИПЫ ПОВТОРЕНИЙ (recurrence_type):
    - 'daily'   → Каждые N дней
    - 'weekly'  → Каждые N недель (можно указать дни: MO, TU, WE, TH, FR, SA, SU)
    - 'monthly' → Каждые N месяцев (день месяца 1-31)

АРХИТЕКТУРА ПОВТОРЯЮЩИХСЯ СОБЫТИЙ:
    ┌────────────────┐
    │ recurring_patterns │  ← Паттерн повторения (type, interval, days_of_week)
    └────────┬───────┘
             │
             │ 1:N
             ▼
    ┌────────────────┐
    │ recurring_instances │  ← Конкретные даты-экземпляры
    └────────┬───────┘
             │
             │ 1:1
             ▼
    ┌────────────────┐
    │    events       │  ← Обычные события (привязаны к instance)
    └────────────────┘

КЛЮЧЕВЫЕ МЕТОДЫ (планируемые):
    | Метод                    | Описание                              |
    |--------------------------|---------------------------------------|
    | create_recurring_event() | Создать паттерн + первые экземпляры   |
    | _generate_instances()    | Генерация дат на N дней вперёд        |
    | _create_instance()       | Создание события на конкретную дату   |
    | add_exception()          | Пропустить дату (исключение)          |
    | update_recurring_event() | Обновить this/future/all экземпляры   |

ФОРМАТ days_of_week:
    ['MO', 'TU', 'WE', 'TH', 'FR', 'SA', 'SU']
    Например: ['MO', 'WE', 'FR'] = понедельник, среда, пятница

TODO (что нужно доделать):
    1. Добавить таблицы recurring_patterns и recurring_instances в database.py
    2. Реализовать методы save_recurring_pattern(), get_recurring_pattern()
    3. Реализовать _generate_instances() для всех типов повторений
    4. Реализовать update_recurring_event() с разными типами обновления
    5. Интегрировать с decision_engine.py (обработка "каждый день в 9:00")
...
class RecurringEventsEngine:
    ...
    ⚠️ СТАТУС: В разработке. Большинство методов содержат TODO-заглушки.
    """
